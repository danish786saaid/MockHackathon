        ...
        {suggestions.map((s) => (
          <li key={s.id} className="flex flex-col gap-2 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs text-[#a8a29e]">{s.reason}</p>
            </div>
            <div className="flex shrink-0 gap-2">
              <button
                type="button"
                onClick={() => onAccept(s.id)}
                className="rounded-lg bg-[#ea580c] px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-[#c2410c]"
              >
                Accept
              </button>
              <button
                ...
                className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-medium text-[#a8a29e] transition-colors hover:bg-white/10 hover:text-white"
              >
                Dismiss
              </button>
            </div>
