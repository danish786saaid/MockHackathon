  ...
  return (
    <div className="glass-card overflow-hidden p-6">
      <div className="mb-4 flex flex-col gap-3">
        <h3 className="text-theme-primary text-base font-semibold">Watchlist</h3>
      ...
      <div className="space-y-3">
        {filteredList.map((item) => (
          <div key={item.name} className="glass-subtle flex items-center justify-between px-4 py-3">
            <div>
              <p className="text-theme-primary font-medium">{item.name}</p>
              <p className="text-theme-muted text-xs">{item.exchange}</p>
            </div>
            <div className="text-right">
              <p className="text-theme-primary font-medium">${item.price.toLocaleString("en-US", { minimumFractionDigits: 1 })}</p>
